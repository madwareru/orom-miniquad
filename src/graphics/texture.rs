...
impl From<TextureFormat> for (GLenum, GLenum, GLenum) {
    fn from(format: TextureFormat) -> Self {
        match format {
            ...
            TextureFormat::Alpha => (GL_R8, GL_RED, GL_UNSIGNED_BYTE), // texture updates will swizzle Red -> Alpha
        }
    }
}
...
impl Texture {
    ...
    pub fn set_filter(&self, ctx: &mut Context, filter: FilterMode) {
        ...
        unsafe {
            ...
            glTexParameteri(target, GL_TEXTURE_MAG_FILTER, filter as i32);
        }
        ctx.cache.restore_texture_binding(0);
    }
            ...
            match self.kind {
                ...
                TextureKind::Texture3D => {
                    if self.format == TextureFormat::Alpha {
                        // if alpha miniquad texture, the value is stored in red channel
                        // swizzle red -> alpha
                        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_SWIZZLE_A, GL_RED as _);
                    } else {
                        // keep alpha -> alpha
                        glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_SWIZZLE_A, GL_ALPHA as _);
                    }
